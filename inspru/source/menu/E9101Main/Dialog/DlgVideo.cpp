...
: CDialogBase(CDlgVideo::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CDlgVideo, CDialogBase)
...
//变换语言和单位
BOOL CDlgVideo::OnLanguageChange()//变皮肤
{
...
BOOL CDlgVideo::OnSkinChange()
{
	return TRUE;
}


void CDlgVideo::InitGdi()
{
	CRect rc;
	HDC hDC;
	hDC = ::GetDC(m_hWnd);
...
void CDlgVideo::InitControl()
{
	...
	//视频选择
	m_Item[0].rect.left		= 64;
	m_Item[0].rect.top		= 172;
	m_Item[0].rect.right	= 216;
	m_Item[0].rect.bottom	= 323;
	m_Item[0].chChar		= _T("视频");
	//监控回放
	m_Item[1].rect.left		= 235;
	m_Item[1].rect.top		= 172;
	m_Item[1].rect.right	= 381;
	m_Item[1].rect.bottom	= 323;
	m_Item[1].chChar		= _T("监控回放");
	//查看抓拍
	m_Item[2].rect.left		= 415;
	m_Item[2].rect.top		= 172;
	m_Item[2].rect.right	= 567;
	m_Item[2].rect.bottom	= 323;
	m_Item[2].chChar		= _T("查看抓拍");
	//设置
	m_Item[3].rect.left		= 587;
	m_Item[3].rect.top		= 172;
	m_Item[3].rect.right	= 739;
	m_Item[3].rect.bottom	= 323;
...
void CDlgVideo::DrawItems(CDC* pDC,const CRect &rt,enBtnState state)
{
	ASSERT(pDC);
	int x = 0;
	int y = 0;
...
void CDlgVideo::DrawBtnText(CDC* pDC,CRect &rt,CString &str,CFont& font,COLORREF col,UINT Format ,BOOL bDisable/* =FALSE */)
{
	COLORREF  nOldRgb;

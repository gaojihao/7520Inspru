// DlgInfoMenu.cpp : implementation file
//
...
//是否有新的中心信息
BOOL CDlgInfoMenu::m_bSMSCenter = FALSE;
BOOL CDlgInfoMenu::m_bIsOpen = FALSE;
UINT CDlgInfoMenu::WM_DlgInfoMenu_Refresh = RegisterWindowMessage(_T("WM_DlgInfoMenu_Refresh"));

CDlgInfoMenu::CDlgInfoMenu(BOOL bIsModelDlg /*= TRUE*/, CWnd* pParent /*=NULL*/)
	: CDialogBase(CDlgInfoMenu::IDD, pParent)
{
	m_nPageIndex	= 0;
	m_nPageCount	= 1;
	m_nRecordCount	= 0;
...
void CDlgInfoMenu::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgInfoMenu, CDialogBase)
...
BOOL CDlgInfoMenu::OnInitDialog() 
{
	CDialogBase::OnInitDialog();

	SetWindowText(_T("DlgInfoMenu_WINDOW_NAME"));
...
//变换语言和单位
BOOL CDlgInfoMenu::OnLanguageChange()//变皮肤
{
	CResString str;
...
BOOL CDlgInfoMenu::OnSkinChange()
{
	return TRUE;
}

void CDlgInfoMenu::InitGdi()
{
	CRect rc;
	HDC hDC;
	hDC = ::GetDC(m_hWnd);
...
void CDlgInfoMenu::DrawItems(CDC* pDC,const CRect &rt, enBtnState state)
{
	ASSERT(pDC);
	int x = 0;
	int y = 0;
...
void CDlgInfoMenu::DrawBtnText(CDC* pDC,CRect &rt,CString &str,CFont& font,COLORREF col,UINT Format ,BOOL bDisable/* =FALSE */)
{
	COLORREF  nOldRgb;
...
void CDlgInfoMenu::DrawList(CDC* pmemDC)
{
	/*
	CString str;
	//计算最大页数
...
void CDlgInfoMenu::OnLButtonDown(UINT nFlags, CPoint point) 
{
	int i = 0;
	for( i = 0; i < BTN_COUNT; i++ )
	{
...
void CDlgInfoMenu::OnLButtonUp(UINT nFlags, CPoint point) 
{
	int i=0;
	for( i=0; i<BTN_COUNT; i++)
	{
...
void  CDlgInfoMenu::OnBtnPageDown()
{
...
void  CDlgInfoMenu::OnBtnDelAll()
{
	...
	if( CDlgConfirm::s_bOk )
	{
		...
		db.execDML( "DELETE FROM info_menu;" );
		db.close();
...
void    CDlgInfoMenu::UserClickItem(int nItem )
{
	CString str = _T("信息点播/取消");
	unCOMMON_DEMAND common_demand;

	common_demand.demand_result.wMsgType = m_nID[nItem];
	...
	if( CDlgConfirm::s_bOk )
	{
		common_demand.demand_result.wMsgTag = 1;
	}
	else
	{
		common_demand.demand_result.wMsgTag = 0;
	}

	CMsgQueue::Instance().PostMsgQueue(g_nWM_MENU_GNSSTERMINAL, MSG_TMN_INFO_DEMAND_CANCEL, common_demand.dwDemand_result);

	ShowPage( m_nPageIndex );
	Invalidate();
}

void    CDlgInfoMenu::ShowPage(int nPageIndex)
{
	CString str;
	int i = 0;
	int nStartIndex = 0;
	...
	//查询记录总数量
	m_nRecordCount = db.execScalar("SELECT count(*) FROM info_menu;");
	//计算总页数
	if(m_nRecordCount > 0)
	...
	sprintf(szSqlBuffer, "SELECT * FROM info_menu ORDER BY UID DESC LIMIT %d, %d;", nPageIndex*elist_count, elist_count);
	...
	for( i = 0; i < elist_count; i++ )
	{
		if ( !q.eof() )	//数据行
		{
			m_nID[i]			= q.getIntField("type");
			m_list[i].chChar	= q.fieldValue("info_name");
			m_list[i].nState	= BTN_STATE_NORMAL;
			q.nextRow();
		}
		else			//空白行
...
LRESULT CDlgInfoMenu::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CDlgInfoMenu::ChangeReadStatus()
{
	CppSQLite3DB db;
	db.open(PATH_SQLITE_DB_808);	//打开数据库
...
void CDlgInfoMenu::PostNcDestroy()
{

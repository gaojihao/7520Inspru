...
//变换语言和单位
BOOL CSMSInfo::OnLanguageChange()//变皮肤
{
	CResString str;
...
//	m_bSavedMsg = bSaved;
	...
	sprintf(szSqlBuffer, "UPDATE sms SET read_status = 1 WHERE UID = '%d';", m_nId); //更新状态
	db.execDML(szSqlBuffer);

	sprintf(szSqlBuffer, "SELECT * FROM sms WHERE UID = '%d';", nIdx);
	q = db.execQuery(szSqlBuffer);

	const char* number = q.fieldValue("phone_number");	//电话号码
	m_Static[0].chChar = number;
	m_Static[1].chChar = q.fieldValue("sms_datatime");	//时间
	m_Static[2].chChar = q.fieldValue("sms_content");	//文本内容

	sprintf(szSqlBuffer, "SELECT * FROM phone_book where phone_number = '%s' ;", number);
	p = db.execQuery(szSqlBuffer);
	CString strName = _T("");
	strName = p.fieldValue("name");		//名字
	if (strName != _T(""))
	{

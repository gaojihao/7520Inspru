...
{
public:
	stnet_param   m_stParamNet;
	sttime_param  m_stParamTime;
	stsvr_msg     m_stSvrMsg;
	stspeed_alert m_stSeedAlert;
	stlisten_phone m_stlisten;
	storder_confirm  m_stcmd_ack;
	static stpos_request m_st_pos_request;

	char          m_call_num[32];

public:
	bool   fht_ack( const char* p_buf, int n_size );
...
protected:
	...
	int   parse_null(const char* p_buf, int n_len);
	int   parse_svr_param(const char* p_buf, int n_len);
	void  parse_sim(const char* p_buf);
	void  parse_register_gsm(const char* p_buf);
	void  parse_register_gprs(const char* p_buf);
	void  parse_connect_ip(const char* p_buf);
	void  parse_Dispatch_msg(const char* p_buf, int n_len);

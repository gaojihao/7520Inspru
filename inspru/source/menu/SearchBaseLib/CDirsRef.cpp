...
void CDirsRef::Load(CString ParentDirName,UINT recursion,CString fext)
{
	if(!m_vDirPath.IsEmpty())
		m_vDirPath.RemoveAll();
	if(!m_vFilePath.IsEmpty())
...
CDirsRef::GetFileAndDir(CString ParentDirName,UINT recursion,CString fext)
{
	...
	int DelPathLen = ParentDirName.GetLength() + 1;
	//是否递归查找
	o.dirs(recursion);
...
void
CDirsRef::UnisSerialize(CArchive& ar)
{
	if(ar.IsStoring())
	{
		ar << m_iDirCnt;
